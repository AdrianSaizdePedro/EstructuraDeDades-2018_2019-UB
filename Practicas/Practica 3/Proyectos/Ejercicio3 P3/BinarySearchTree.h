...
int BinarySearchTree<E>::height(NodeTree<E>* p) const {
    if (p->isExternal()) {
        ...
        return max(height(p->right()), height(p->left()));
    }
...
void BinarySearchTree<E>::nodeDesequilibrio(NodeTree<E>* p){
    ...
    while (aux2 != this->rootB) {
        ...
        //Left Left Case
        if(BF>1 && p->getElement()<aux2->left()->getElement()){
            this->rightRotation(aux2);
        }   
        //Right Right Case
        else if(BF<-1 && p->getElement()> aux2->right()->getElement()){
            cout<<"3,5"<<endl;
            this->leftRotation(aux2);
        }

        //Left Right Case
        else if(BF>1 && p->getElement()>aux2->left()->getElement()){
